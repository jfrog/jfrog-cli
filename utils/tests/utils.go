...
func init() {
	...
	TestAgentApm = flag.Bool("test.agentApm", false, "Test Agent APM (Agent Package Manager)")
...
var reposConfigMap = map[*string]string{
	...
	&AgentApmLocalRepo:              AgentApmLocalRepositoryConfig,
...
func GetNonVirtualRepositories() map[*string]string {
	nonVirtualReposMap := map[*bool][]*string{
		...
		TestAgentApm:           {&AgentApmLocalRepo},
...
func GetBuildNames() []string {
	buildNamesMap := map[*bool][]*string{
		...
		TestAgentApm:     {&AgentApmBuildName},
...
func getSubstitutionMap() map[string]string {
	return map[string]string{
		...
		"${AGENT_APM_LOCAL_REPO}":      AgentApmLocalRepo,
...
func AddTimestampToGlobalVars() {
	...
	AgentApmBuildName += uniqueSuffix
