...
var reposConfigMap = map[*string]string{
	...
	&LifecycleImportDependency: "import_bundle_repo_depedency.json",
}
...
func GetNonVirtualRepositories() map[*string]string {
	nonVirtualReposMap := map[*bool][]*string{
		TestArtifactory:        {&RtRepo1, &RtRepo2, &RtLfsRepo, &RtDebianRepo, &TerraformRepo, &LifecycleImportDependency},
