...
func GetCommands() []cli.Command {
	cmds := cliutils.GetSortedCommands(cli.CommandsByName{
		...
		},
		{
			...
			ArgsUsage:       common.CreateEnvVars(),
			SkipFlagParsing: true,
			BashComplete:    corecommon.CreateBashCompletionFunc("upload", "u", "add", "upgrade", "update", "fetch", "search", "del", "info"),
			Category:        buildToolsCategory,
			Action:          ApkCmd,
		},
		{
...
// ApkCmd dispatches jf apk subcommands: upload and native apk operations.
func ApkCmd(c *cli.Context) error {
	...
	// "u" is a shortcut for the "upload" subcommand.
	if subcmd == "u" {
		subcmd = "upload"
	}
	...
	var serverDetails *coreConfig.ServerDetails
	if serverID != "" {
		// User explicitly named a server — fail fast if it doesn't exist.
		serverDetails, err = coreConfig.GetSpecificConfig(serverID, false, false)
		if err != nil {
			return errorutils.CheckErrorf("server ID %q not found in configuration. "+
