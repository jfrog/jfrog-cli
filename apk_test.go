...
func TestApkAdd_ArtifactoryUnreachable(t *testing.T) {
	...
	const unreachableServerID = "apk-unreachable-server"
	configCli := coretests.NewJfrogCli(execMain, "jfrog config", "")
	require.NoError(t, configCli.Exec("add", unreachableServerID, "--interactive=false",
		"--artifactory-url=http://127.0.0.1:1/artifactory",
		"--user=admin", "--password=password", "--enc-password=false", "--overwrite"),
		"failed to configure a server pointing at an unreachable address")
	defer func() { _ = configCli.Exec("rm", unreachableServerID, "--quiet") }()

	jfrogCli := coretests.NewJfrogCli(execMain, "jfrog", "")
	err := jfrogCli.Exec("apk", "add", "curl",
		"--repo="+tests.AlpineVirtualRepo,
		"--server-id="+unreachableServerID)
	require.Error(t, err, "jf apk add should fail when Artifactory is unreachable, not fall back to the public CDN")
	assert.NotContains(t, strings.ToLower(err.Error()), "dl-cdn.alpinelinux.org",
		"the failure should come from the configured Artifactory, not the public CDN")
}
