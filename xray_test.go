...
func TestXrayAuditNpm(t *testing.T) {
	...
	err = fileutils.CopyDir(npmProjectPath, tempDirPath, true, nil)
	assert.NoError(t, err)
...
func TestXrayAuditGradle(t *testing.T) {
	...
	err = fileutils.CopyDir(gradleProjectPath, tempDirPath, true, nil)
	assert.NoError(t, err)
	prevWd := changeWD(t, tempDirPath)
	defer os.Chdir(prevWd)
...
func TestXrayAuditMaven(t *testing.T) {
	...
	err = fileutils.CopyDir(mvnProjectPath, tempDirPath, true, nil)
	assert.NoError(t, err)
	prevWd := changeWD(t, tempDirPath)
	defer os.Chdir(prevWd)
