...
func uploadCmd(c *cli.Context) (err error) {
	...
	printDeploymentView, detailedSummary := log.IsStdErrTerminal(), c.Bool("detailed-summary") || commandsummary.ShouldRecordSummary()
...
func buildPublishCmd(c *cli.Context) error {
	...
	buildPublishCmd := buildinfo.NewBuildPublishCommand().SetServerDetails(rtDetails).SetBuildConfiguration(buildConfiguration).SetConfig(buildInfoConfiguration).SetDetailedSummary(c.Bool("detailed-summary") || commandsummary.ShouldRecordSummary())
