...
func getFileInfo(artifactPath string, flags *BuildAddArtifactFlags) (fileName string, details *fileutils.FileDetails, err error) {
	...
	resp, body, _, err := httputils.SendGet(getFileInfoUrl, true, httpClientsDetails); if err != nil {
		return "", nil, err
	}
...
func setBuildNameAndNumber(buildName, buildNumber, artifactPath string, flags *BuildAddArtifactFlags) (err error) {
