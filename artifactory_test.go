...
func testMoveCopySpec(command string, t *testing.T) {
	...
	isExistInArtifactory(tests.GetMoveCopySpecExpected(), searchMovedCopiedSpec, t)
	...
	var expected []string
	// Verify original files still exist if used copy
	if command == "copy" {
		expected = tests.GetRepo1TestResourcesExpected()
	}
	isExistInArtifactory(expected, searchOriginalSpec, t)
	cleanArtifactoryTest()
}
...
func TestArtifactoryUploadFlatFolderWithFileAndInnerEmptyMatchingPattern(t *testing.T) {
	...
	if !fileutils.IsPathExists(tests.Out+"/d", false) {
		t.Error("bottom chain directory, 'd', is missing")
	}
	// Cleanup
	cleanArtifactoryTest()
}
...
func TestArtifactoryFolderUploadNonRecursive(t *testing.T) {
	...
	if !fileutils.IsPathExists(tests.Out+"/folder", false) {
		t.Error("Failed to download folder from Artifactory")
	}
...
func testChecksumDownload(t *testing.T, outFileName string) {
	...
	if !exists {
		t.Error("Failed to download file from Artifactory")
	}
