...
func testArtifactoryDeleteFoldersNoSpec(t *testing.T, contentOnly bool) {
	...
	resp, body, _, err := client.SendGet(artifactoryDetails.Url+"api/storage/"+tests.RtRepo1+"/test_resources", true, artHttpDetails)
	assert.NoError(t, err)
...
func execListBuildNamesRest() ([]string, error) {
	...
	// Send get request
	resp, body, _, err := client.SendGet(artifactoryDetails.Url+"api/build", true, artHttpDetails)
	if err != nil {
		return nil, err
	}
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
...
func TestVcsProps(t *testing.T) {
	initArtifactoryTest(t)
	defer inttestutils.DeleteBuild(artifactoryDetails.Url, tests.RtBuildName1, artHttpDetails)
...
func TestRefreshableTokens(t *testing.T) {
	...
	curAccessToken, curRefreshToken, err := getTokensFromConfig(tests.RtServerId)
	if err != nil {
		assert.NoError(t, err)
		return
	}
	...
	newAccessToken, newRefreshToken, err := getTokensFromConfig(tests.RtServerId)
	if err != nil {
		assert.NoError(t, err)
		return
	}
...
func getTokensFromConfig(serverId string) (accessToken, refreshToken string, err error) {
...
func assertTokensChanged(t *testing.T, serverId, curAccessToken, curRefreshToken string) (newAccessToken, newRefreshToken string, err error) {
	newAccessToken, newRefreshToken, err = getTokensFromConfig(serverId)
	if err != nil {
		return "", "", err
	}
