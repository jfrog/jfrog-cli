...
func CliExecAndAssert(t *testing.T, args ...string) {
...
func TestArtifactorySimpleUploadSpec(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "--spec="+specFile)
...
func TestArtifactoryUploadPathWithSpecialCharsAsNoRegex(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1, "--flat")
	searchFilePath, err := tests.CreateSpec(tests.SearchRepo1ByInSuffix)
	assert.NoError(t, err)
...
func TestArtifactoryEmptyBuild(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "*.notExist", tests.RtRepo1, "--build-name="+tests.RtBuildName1, "--build-number="+buildNumber)

	// Try to download with non existent pattern
	CliExecAndAssert(t, "download", tests.RtRepo1+"/*.notExist", "--build-name="+tests.RtBuildName1, "--build-number="+buildNumber)
...
func TestArtifactoryPublishBuildUsingBuildlFile(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filepath.Join(wdCopy, "testdata", "a", "a1.in"), tests.RtRepo1+"/foo")

	// Publish build-info using the build.yaml file.
	CliExecAndAssert(t, "build-publish")
	...
	CliExecAndAssert(t, "upload", filepath.Join(wdCopy, "testdata", "a", "a1.in"), tests.RtRepo1+"/bla-bla")
...
func TestArtifactoryDownloadFromVirtual(t *testing.T) {
	...
	CliExecAndAssert(t, "dl", tests.RtVirtualRepo+"/testdata/(*)", tests.Out+"/"+"{1}", "--flat=true")
...
func TestArtifactoryDownloadPathWithSpecialChars(t *testing.T) {
	initArtifactoryTest(t)
	CliExecAndAssert(t, "upload", getSpecialCharFilePath(), tests.RtRepo1, "--flat=false")
	CliExecAndAssert(t, "upload", "testdata/c#/a#1.in", tests.RtRepo1, "--flat=false")

	CliExecAndAssert(t, "dl", tests.RtRepo1+"/testdata/a$+~&^a#/a*", tests.Out+fileutils.GetFileSeparator(), "--flat=true")
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/testdata/c#/a#1.in", tests.Out+fileutils.GetFileSeparator(), "--flat=true")
...
func TestArtifactoryDownloadPatternWithUnicodeChars(t *testing.T) {
	initArtifactoryTest(t)
	CliExecAndAssert(t, "upload", "testdata/unicode/", tests.RtRepo1, "--flat=false")
	...
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/testdata/unicode/(dirλrectory)/", filepath.Join(tests.Out, "{1}")+fileutils.GetFileSeparator(), "--flat=true")
...
func testArtifactoryDownload(fileSize int, t *testing.T) {
	...
	CliExecAndAssert(t, "u", randFile.Name(), tests.RtRepo1+"/testdata/", "--flat=true")
	assert.NoError(t, randFile.File.Close())
	assert.NoError(t, os.RemoveAll(tests.Out))
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/testdata/", tests.Out+fileutils.GetFileSeparator(), "--flat=true")
...
func TestArtifactoryDownloadWildcardInRepo(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/a1.in")
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo2+"/path/a2.in")
	...
	CliExecAndAssert(t, "dl", "--spec="+specFile)
	paths, err := fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	assert.NoError(t, err)
...
func TestArtifactoryDownloadPlaceholderInRepo(t *testing.T) {
	...
	CliExecAndAssert(t, "dl", tests.RtRepo1And2Placeholder, tests.Out+"/a/{1}/", "--flat=true")
	paths, err := fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	assert.NoError(t, err)
...
func TestArtifactoryUploadPlaceholderFlat(t *testing.T) {
	...
	for _, flatValue := range []string{"true", "false"} {
		CliExecAndAssert(t, "upload", "testdata/a/b/(*)", tests.RtRepo1+"/path/{1}", "--flat="+flatValue)
		...
		CliExecAndAssert(t, "del", tests.RtRepo1+"/*")
		...
		CliExecAndAssert(t, "upload", "testdata/a/b/(*)/(*)", tests.RtRepo1+"/path/{1}/{2}", "--flat="+flatValue)
		searchPath, err = tests.CreateSpec(tests.SearchAllRepo1)
		assert.NoError(t, err)
...
func TestArtifactoryDownloadWithPlaceholderFlat(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/a/b/(*)", tests.RtRepo1+"/path/{1}")
	// Download the tests data using placeholder with flat
	for _, flatValue := range []string{"true", "false"} {
		CliExecAndAssert(t, "download", tests.RtRepo1+"/path/(*)", tests.Out+"/mypath2/{1}", "--flat="+flatValue)
		paths, err := fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
		assert.NoError(t, err)
		...
		CliExecAndAssert(t, "download", tests.RtRepo1+"/path/(*)/(*)", tests.Out+"/mypath2/{1}/{2}", "--flat="+flatValue)
		paths, err = fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
		assert.NoError(t, err)
...
func TestArtifactoryCopyWithPlaceholderFlat(t *testing.T) {
	initArtifactoryTest(t)
	// Upload test data to Artifactory
	CliExecAndAssert(t, "upload", "testdata/a/b/(*)", tests.RtRepo2+"/mypath2/{1}")
	// Download the tests data using placeholder with flat
	for _, flatValue := range []string{"true", "false"} {
		CliExecAndAssert(t, "copy", tests.RtRepo2+"/mypath2/(*)", tests.RtRepo1+"/path/{1}", "--flat="+flatValue)
		searchPath, err := tests.CreateSpec(tests.SearchAllRepo1)
		assert.NoError(t, err)
		verifyExistInArtifactory(tests.GetUploadedFileWithDownloadedPlaceHolder(), searchPath, t)
		CliExecAndAssert(t, "del", tests.RtRepo1+"/*")
		...
		CliExecAndAssert(t, "copy", tests.RtRepo2+"/mypath2/(*)/(*)", tests.RtRepo1+"/path/{1}/{2}", "--flat="+flatValue)
		searchPath, err = tests.CreateSpec(tests.SearchAllRepo1)
		assert.NoError(t, err)
		verifyExistInArtifactory(tests.GetUploadedFileWithDownloadedDoublePlaceHolder(), searchPath, t)
		CliExecAndAssert(t, "del", tests.RtRepo1+"/*")
...
func TestArtifactoryMoveWithPlaceholderFlat(t *testing.T) {
	...
	for _, flatValue := range []string{"true", "false"} {
		CliExecAndAssert(t, "upload", "testdata/a/b/(*)", tests.RtRepo2+"/mypath2/{1}")
		CliExecAndAssert(t, "move", tests.RtRepo2+"/mypath2/(*)", tests.RtRepo1+"/path/{1}", "--flat="+flatValue)
		searchPath, err := tests.CreateSpec(tests.SearchAllRepo1)
		assert.NoError(t, err)
		verifyExistInArtifactory(tests.GetUploadedFileWithDownloadedPlaceHolder(), searchPath, t)
		CliExecAndAssert(t, "del", tests.RtRepo1+"/*")
		CliExecAndAssert(t, "del", tests.RtRepo2+"/*")
		...
		CliExecAndAssert(t, "upload", "testdata/a/b/(*)", tests.RtRepo2+"/mypath2/{1}")
		CliExecAndAssert(t, "move", tests.RtRepo2+"/mypath2/(*)/(*)", tests.RtRepo1+"/path/{1}/{2}", "--flat="+flatValue)
		searchPath, err = tests.CreateSpec(tests.SearchAllRepo1)
		assert.NoError(t, err)
		verifyExistInArtifactory(tests.GetUploadedFileWithDownloadedDoublePlaceHolder(), searchPath, t)
		CliExecAndAssert(t, "del", tests.RtRepo1+"/*")
...
func TestArtifactoryCopySingleFileNonFlat(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/", "--flat")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/path/a1.in", tests.RtRepo2)
...
func TestArtifactoryCopyPrefixFilesFlat(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/prefix/(*)", tests.RtRepo1+"/prefix/prefix-{1}")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/prefix/*", tests.RtRepo2, "--flat")
	searchPath, err := tests.CreateSpec(tests.SearchRepo2)
	assert.NoError(t, err)
...
func TestAqlFindingItemOnRoot(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/inner/", "--flat=true")
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/someFile", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/*", tests.RtRepo2)
	searchPath, err := tests.CreateSpec(tests.SearchRepo2)
	assert.NoError(t, err)
	verifyExistInArtifactory(tests.GetAnyItemCopy(), searchPath, t)
	CliExecAndAssert(t, "del", tests.RtRepo2+"/*")
	CliExecAndAssert(t, "del", tests.RtRepo1+"/*")
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/", "--flat=true")
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/someFile", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/*/", tests.RtRepo2)
	verifyExistInArtifactory(tests.GetSingleFileCopy(), searchPath, t)
	cleanArtifactoryTest()
...
func TestExitCode(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", path.Join("testdata", "a", "a1.in"), tests.RtRepo1)
...
func TestArtifactoryDirectoryCopy(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/path/", tests.RtRepo2)
	searchPath, err := tests.CreateSpec(tests.SearchRepo2)
	assert.NoError(t, err)
	verifyExistInArtifactory(tests.GetSingleFileCopy(), searchPath, t)
	cleanArtifactoryTest()
...
func TestArtifactoryCopyFilesNameWithParentheses(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/b/*", tests.RtRepo1, "--flat=false")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/(/(.in", tests.RtRepo2)
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/(b/(b.in", tests.RtRepo2)
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/b(/b(.in", tests.RtRepo2)
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/b)/b).in", tests.RtRepo2)
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/(b)/(b).in", tests.RtRepo2)
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/)b/)b.in", tests.RtRepo2)
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/)b)/)b).in", tests.RtRepo2)
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/(b/(b.in", tests.RtRepo2+"/()/", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/(b)/(b).in", tests.RtRepo2+"/()/")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/b(/b(.in", tests.RtRepo2+"/(/", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/(/(*.in)", tests.RtRepo2+"/c/{1}.zip", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/(/(*.in)", tests.RtRepo2+"/(/{1}.zip")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/b(/(b*.in)", tests.RtRepo2+"/(/{1}-up", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/testdata/b/b(/(*).(*)", tests.RtRepo2+"/(/{2}-{1}", "--flat=true")
...
func TestArtifactoryCreateUsers(t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "users-create", "--csv="+randomUsersCSVPath)
	// Clean up
	defer func() {
		CliExecAndAssert(t, "users-delete", "--csv="+randomUsersCSVPath)
		cleanArtifactoryTest()
	}()
...
func TestArtifactoryDownloadFilesNameWithParenthesis(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/b/*", tests.RtRepo1, "--flat=false")
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1), tests.Out+"/")
...
func TestArtifactoryDownloadDotAsTarget(t *testing.T) {
	...
	assert.NoError(t, randFile.File.Close())
	CliExecAndAssert(t, "upload", tests.Out+"/*", tests.RtRepo1+"/p-modules/", "--flat=true")
	assert.NoError(t, os.RemoveAll(tests.Out))
	assert.NoError(t, fileutils.CreateDirIfNotExist(tests.Out))
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/p-modules/DownloadDotAsTarget", ".")
	chdirCallback()
...
func TestArtifactoryDirectoryCopyUsingWildcardFlat(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/inner/", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/path/inner", tests.RtRepo2, "--flat=true")
	searchPath, err := tests.CreateSpec(tests.SearchRepo2)
	assert.NoError(t, err)
...
func TestArtifactoryCopyPathsTwice(t *testing.T) {
	initArtifactoryTest(t)
	var filePath = getSpecialCharFilePath()
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/inner/", "--flat")
	...
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/path", tests.RtRepo2+"/path")
	verifyExistInArtifactory(tests.GetSingleFileCopyFullPath(), searchPath, t)
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/path", tests.RtRepo2+"/path")
...
func TestArtifactoryDirectoryCopyPatternEndsWithSlash(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/inner/")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/path/", tests.RtRepo2, "--flat=true")
	searchPath, err := tests.CreateSpec(tests.SearchRepo2)
	assert.NoError(t, err)
...
func TestArtifactoryCopyAnyItemRecursive(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/a/b/", "--flat")
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/aFile", "--flat=true")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/a*", tests.RtRepo2, "--recursive=true")
	searchPath, err := tests.CreateSpec(tests.SearchRepo2)
	assert.NoError(t, err)
...
func TestArtifactoryCopyAndRenameFolder(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/inner/", "--flat")
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/path/(*)", tests.RtRepo2+"/newPath/{1}")
	searchPath, err := tests.CreateSpec(tests.SearchRepo2)
	assert.NoError(t, err)
...
func TestArtifactoryCopyAnyItemUsingSpec(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/path/inner/")
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1+"/someFile", "--flat=true")
	CliExecAndAssert(t, "cp", "--spec="+specFile)
	verifyExistInArtifactory(tests.GetAnyItemCopyUsingSpec(), searchPath, t)
	cleanArtifactoryTest()
}
...
func TestArtifactoryCopyExcludeByCli(t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+specFileA)
	CliExecAndAssert(t, "upload", "--spec="+specFileB)

	// Copy by pattern
	CliExecAndAssert(t, "cp", tests.RtRepo1+"/data/", tests.RtRepo2+"/", "--exclusions=*/*b*;*/*c*")
...
func TestArtifactoryUploadDebian(t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+specFile, "--deb=bionic/main/i386")
	...
	CliExecAndAssert(t, "upload", "--spec="+specFile, "--deb=cosmic/main\\/18.10/amd64")
...
func TestArtifactoryUploadAndExplode(t *testing.T) {
	initArtifactoryTest(t)
	CliExecAndAssert(t, "upload", filepath.Join("testdata", "archives", "a.zip"), tests.RtRepo1, "--explode=true", "--flat")
	searchFilePath, err := tests.CreateSpec(tests.SearchRepo1ByInSuffix)
	assert.NoError(t, err)
...
func TestArtifactoryUploadAndSyncDelete(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", path.Join("testdata", "a", "*"), tests.RtRepo1+"/syncDir/", "--flat=false")
	...
	CliExecAndAssert(t, "upload", path.Join("testdata", "a", "b", "*1.in"), tests.RtRepo1+"/syncDir/", "--sync-deletes="+tests.RtRepo1+"/syncDir/testdata/a/b/", "--flat=false")
	searchFilePath, err = tests.CreateSpec(tests.SearchAllRepo1)
	assert.NoError(t, err)
	...
	CliExecAndAssert(t, "upload", path.Join("testdata", "archives", "*"), tests.RtRepo1+"/syncDir/", "--sync-deletes="+tests.RtRepo1+"/syncDir/", "--flat=true")
	searchFilePath, err = tests.CreateSpec(tests.SearchAllRepo1)
	assert.NoError(t, err)
	...
	CliExecAndAssert(t, "upload", path.Join("testdata", "c", "*"), tests.RtRepo1+"/syncDir/", "--sync-deletes="+tests.RtRepo1+"/syncDir/", "--flat=false")
	searchFilePath, err = tests.CreateSpec(tests.SearchAllRepo1)
	assert.NoError(t, err)
...
func TestArtifactoryDownloadAndExplode(t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", tests.Out+"/*", tests.RtRepo1, "--flat=true")
	assert.NoError(t, randFile.File.Close())
	assert.NoError(t, os.RemoveAll(tests.Out))
	// Download 'concurrent.tar.gz' as 'concurrent' file name and explode it.
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "concurrent.tar.gz"), tests.Out+"/concurrent", "--explode=true")
	// Download 'concurrent.tar.gz' and explode it.
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "concurrent.tar.gz"), tests.Out+"/", "--explode=true")
	// Download 'concurrent.tar.gz' without explode it.
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "concurrent.tar.gz"), tests.Out+"/", "--explode=false")
	// Try to explode the archive that already been downloaded.
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "concurrent.tar.gz"), tests.Out+"/", "--explode=true")
	assert.NoError(t, os.RemoveAll(tests.Out))
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "randFile"), tests.Out+"/", "--explode=true")
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "concurrent.tar.gz"), tests.Out+"/", "--explode=false")
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "bulk.tar"), tests.Out+"/", "--explode=true")
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "zipFile.zip"), tests.Out+"/", "--explode=true")
...
func TestArtifactoryDownloadAndExplodeCurDirAsTarget(t *testing.T) {
	...
	// Dot as target
	CliExecAndAssert(t, "download", tests.RtRepo1+"/p-modules/curDir.tar.gz", ".", "--explode=true")
...
func TestArtifactoryDownloadAndExplodeFlat(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", tests.Out+"/*", tests.RtRepo1+"/checkFlat/dir/", "--flat=true")
	assert.NoError(t, file1.File.Close())
	assert.NoError(t, os.RemoveAll(tests.Out))
	assert.NoError(t, fileutils.CreateDirIfNotExist(tests.Out))

	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "checkFlat", "dir", "flat.tar"), tests.Out+"/checkFlat/", "--explode=true", "--flat=true", "--min-split=50")
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "checkFlat", "dir", "tarZipFile.zip"), tests.Out+"/", "--explode=true", "--flat=false")
	...
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "checkFlat", "dir", "tarZipFile.zip"), tests.Out+"/", "--explode=true", "--flat=false")
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "checkFlat", "dir", "flat.tar"), tests.Out+"/checkFlat/dir/", "--explode=true", "--flat", "--min-split=50")
...
func TestArtifactoryDownloadAndExplodeConcurrent(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", path.Join("testdata", "archives", "a.zip"), tests.RtRepo1, "--flat=true")
	assert.NoError(t, fileutils.CreateDirIfNotExist(tests.Out))
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "a.zip"), tests.Out+"/", "--explode=true", "--min-split=50")
	verifyExistAndCleanDir(t, tests.GetExtractedDownloadConcurrent)
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "a.zip"), tests.Out+"/", "--explode=false", "--min-split=50")
	verifyExistAndCleanDir(t, tests.GetArchiveConcurrent)
	CliExecAndAssert(t, "download", path.Join(tests.RtRepo1, "a.zip"), tests.Out+"/", "--explode=true", "--split-count=15", "--min-split=50")
	verifyExistAndCleanDir(t, tests.GetExtractedDownloadConcurrent)
...
func TestArtifactoryDownloadAndExplodeSpecialChars(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", tests.Out+"/*", tests.RtRepo1+"/dir/", "--flat=true")
	assert.NoError(t, os.RemoveAll(tests.Out))
	err = fileutils.CreateDirIfNotExist(tests.Out)
	assert.NoError(t, err)
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/dir/a$+~&^a#.tar", tests.Out+"/dir $+~&^a# test/", "--flat=true", "--explode")
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/dir/a$+~&^a#.tar", tests.Out+"/dir $+~&^a# test/", "--flat=false", "--explode")
	verifyExistAndCleanDir(t, tests.GetExtractedDownloadTarFileSpecialChars)
	// Concurrently download
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/dir/a$+~&^a#.tar", tests.Out+"/dir $+~&^a# test/", "--flat=true", "--explode", "--min-split=50")
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/dir/a$+~&^a#.tar", tests.Out+"/dir $+~&^a# test/", "--flat=false", "--explode", "--min-split=50")
	verifyExistAndCleanDir(t, tests.GetExtractedDownloadTarFileSpecialChars)
	cleanArtifactoryTest()
}
...
func TestArtifactoryUploadAsArchive(t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+uploadSpecFile)
	searchFilePath, err := tests.CreateSpec(tests.SearchAllRepo1)
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "download", "--spec="+downloadSpecFile)
	paths, err := fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	assert.NoError(t, err)
...
func TestArtifactoryUploadAsArchiveWithExplodeAndSymlinks(t *testing.T) {
	...
	uploadSpecFile, err := tests.CreateSpec(tests.UploadAsArchive)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+uploadSpecFile, "--symlinks", "--explode")
...
func TestArtifactoryDownloadAndSyncDeletes(t *testing.T) {
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/syncDir/", tests.Out+"/")
	paths, err := fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	assert.NoError(t, err)
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/syncDir/", outDirPath, "--flat=true", "--sync-deletes="+outDirPath)
	paths, err = fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	assert.NoError(t, err)
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/syncDir/*2.in", outDirPath, "--flat=true", "--sync-deletes="+outDirPath)
	paths, err = fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	assert.NoError(t, err)
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/syncDir/", outDirPath, "--sync-deletes="+outDirPath+"syncDir"+string(os.PathSeparator), "--exclusions=*/syncDir/testdata/*c*in")
	...
	CliExecAndAssert(t, "delete", tests.RtRepo1+"/syncDir/")
	searchFilePath, err = tests.CreateSpec(tests.SearchAllRepo1)
	assert.NoError(t, err)
	verifyDoesntExistInArtifactory(searchFilePath, t)

	// Upload all testdata/archives/ to repo1/syncDir/
	CliExecAndAssert(t, "upload", path.Join("testdata", "archives", "*"), tests.RtRepo1+"/syncDir/", "--flat=false")
	searchFilePath, err = tests.CreateSpec(tests.SearchAllRepo1)
	assert.NoError(t, err)
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/syncDir/", outDirPath, "--sync-deletes="+outDirPath+"syncDir"+string(os.PathSeparator))
	paths, err = fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out+string(os.PathSeparator)+"syncDir"+string(os.PathSeparator), false)
	assert.NoError(t, err)
...
func TestArtifactorySetPropertiesOnSpecialCharsArtifact(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/a/a1.in", targetPath)
	// Set the 'prop=red' property to the file.
	CliExecAndAssert(t, "sp", targetPath, "prop=red")
...
func TestArtifactorySetPropertiesExcludeByCli(t *testing.T) {
	initArtifactoryTest(t)
	CliExecAndAssert(t, "upload", "testdata/a/a*.in", tests.RtRepo1+"/")
	CliExecAndAssert(t, "sp", tests.RtRepo1+"/*", "prop=val", "--exclusions=*/*a1.in;*/*a2.in")
	resultItems := searchItemsInArtifactory(t, tests.SearchRepo1ByInSuffix)
	assert.NotZero(t, len(resultItems), "No artifacts were found.")
	for _, item := range resultItems {
		if item.Name != "a3.in" {
...
func TestArtifactoryDeleteProperties(t *testing.T) {
	initArtifactoryTest(t)
	CliExecAndAssert(t, "upload", "testdata/a/a*.in", tests.RtRepo1+"/a/")
	CliExecAndAssert(t, "sp", tests.RtRepo1+"/a/*", "color=yellow;prop=red;status=ok")
	// Delete the 'color' property.
	CliExecAndAssert(t, "delp", tests.RtRepo1+"/a/*", "color")
	// Delete the 'status' property, by a spec which filters files by 'prop=red'.
	specFile, err := tests.CreateSpec(tests.SetDeletePropsSpec)
	assert.NoError(t, err)
	CliExecAndAssert(t, "delp", "status", "--spec="+specFile)
...
func TestArtifactoryDeletePropertiesWithExclude(t *testing.T) {
	initArtifactoryTest(t)
	CliExecAndAssert(t, "upload", "testdata/a/a*.in", tests.RtRepo1+"/")
	CliExecAndAssert(t, "sp", tests.RtRepo1+"/*", "prop=val")

	CliExecAndAssert(t, "delp", tests.RtRepo1+"/*", "prop", "--exclusions=*/*a1.in;*/*a2.in")
	resultItems := searchItemsInArtifactory(t, tests.SearchRepo1ByInSuffix)
	assert.NotZero(t, len(resultItems), "No artifacts were found.")
...
func TestArtifactoryUploadFromHomeDir(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", testFileRel, tests.RtRepo1, "--recursive=false", "--flat=true")
...
func TestArtifactoryUploadExcludeByCli1Wildcard(t *testing.T) {
	initArtifactoryTest(t)
	// Upload files
	CliExecAndAssert(t, "upload", "testdata/a/a*", tests.RtRepo1, "--exclusions=*a2*;*a3.in", "--flat=true")
	searchFilePath, err := tests.CreateSpec(tests.SearchRepo1ByInSuffix)
	assert.NoError(t, err)
...
func TestArtifactoryUploadExcludeByCli1Regex(t *testing.T) {
	initArtifactoryTest(t)
	// Upload files
	CliExecAndAssert(t, "upload", "testdata/a/a(.*)", tests.RtRepo1, "--exclusions=(.*)a2.*;.*a3.in", "--regexp=true", "--flat=true")
	searchFilePath, err := tests.CreateSpec(tests.SearchRepo1ByInSuffix)
	assert.NoError(t, err)
	verifyExistInArtifactory(tests.GetSimpleUploadSpecialCharNoRegexExpectedRepo1(), searchFilePath, t)
	cleanArtifactoryTest()
...
func TestArtifactoryUploadExcludeByCli2Wildcard(t *testing.T) {
	...
	// Upload files
	CliExecAndAssert(t, "upload", filepath.ToSlash(absDirPath)+"/*", tests.RtRepo1, "--exclusions=*cliTestFile1*", "--flat=true")
...
func TestArtifactoryUploadExcludeByCli2Regex(t *testing.T) {
	...
	// Upload files
	CliExecAndAssert(t, "upload", filepath.ToSlash(absDirPath)+"(.*)", tests.RtRepo1, "--exclusions=(.*c)liTestFile1.*", "--regexp=true", "--flat=true")
...
func TestArtifactoryUploadWithRegexEscaping(t *testing.T) {
	initArtifactoryTest(t)
	// Upload files
	CliExecAndAssert(t, "upload", "testdata/regexp"+"(.*)"+"\\."+".*", tests.RtRepo1, "--regexp=true", "--flat=true")
	searchFilePath, err := tests.CreateSpec(tests.SearchAllRepo1)
	assert.NoError(t, err)
...
func testMoveCopySpec(command string, t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, command, "--spec="+specFile)
...
func TestValidateValidSymlink(t *testing.T) {
	...
	CliExecAndAssert(t, "u", validLink, tests.RtRepo1, "--symlinks=true", "--flat=true")
	...
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/link", tests.GetTestResourcesPath()+"a/", "--validate-symlinks=true")

	// Should be valid if successful
	validateSymLink(validLink, localFile, t)

	// Delete symlink and clean
	assert.NoError(t, os.Remove(validLink))
	cleanArtifactoryTest()
}
...
func TestValidateBrokenSymlink(t *testing.T) {
	...
	// Upload symlink to artifactory
	CliExecAndAssert(t, "u", linkPath, tests.RtRepo1, "--symlinks=true", "--flat=true")
	...
	if !assert.Error(t, err, "A broken symLink was downloaded although validate-symlinks flag was set to true") {
		assert.NoError(t, os.Remove(linkPath))
	}
...
func TestExcludeBrokenSymlink(t *testing.T) {
	...
	CliExecAndAssert(t, "u", filepath.Join(tests.Out, "*"), tests.RtRepo1, "--symlinks=true", "--exclusions=*")
	cleanArtifactoryTest()
}
...
func TestSymlinkWildcardPathHandling(t *testing.T) {
	...
	CliExecAndAssert(t, "u", link1, tests.RtRepo1, "--symlinks=true", "--flat=true")
	err = os.Remove(link)
	assert.NoError(t, err)
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/link", tests.GetTestResourcesPath()+"a/", "--validate-symlinks=true")
...
func TestUploadWithArchiveAndSymlink(t *testing.T) {
	...
	CliExecAndAssert(t, "u", tmpDir+"/*", tests.RtRepo1+"/test-archive.zip", "--archive=zip", "--symlinks=true", "--flat=true")
	assert.NoError(t, os.RemoveAll(tmpDir))
	assert.NoError(t, os.Mkdir(tmpDir, 0777))
	CliExecAndAssert(t, "download", tests.RtRepo1+"/test-archive.zip", tmpDir+"/", "--explode=true")
	// Validate
...
func TestSymlinkToDirHandling(t *testing.T) {
	...
	err := os.Symlink(localFile, link)
	assert.NoError(t, err)
	CliExecAndAssert(t, "u", link, tests.RtRepo1, "--symlinks=true", "--recursive=true", "--flat=true")
	err = os.Remove(link)
	assert.NoError(t, err)
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/link", tests.GetTestResourcesPath()+"a/")
	validateSymLink(link, localFile, t)
	assert.NoError(t, os.Remove(link))
	cleanArtifactoryTest()
}
...
func TestSymlinkToDirWildcardHandling(t *testing.T) {
	...
	CliExecAndAssert(t, "u", link1, tests.RtRepo1, "--symlinks=true", "--recursive=true", "--flat=true", "--flat=true")
	err = os.Remove(link)
	assert.NoError(t, err)
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/link", tests.GetTestResourcesPath()+"a/")
	validateSymLink(link, localFile, t)
...
func TestSymlinkInsideSymlinkDirWithRecursionIssueUpload(t *testing.T) {
	...
	CliExecAndAssert(t, "u", localDirPath+"/link*", tests.RtRepo1, "--symlinks=true", "--recursive=true", "--flat=true")
	assert.NoError(t, os.Remove(link1))

	assert.NoError(t, os.Remove(link2))
...
func testArtifactorySimpleDelete(t *testing.T, deleteSpecPath string) {
	...
	if deleteSpecPath != "" {
		CliExecAndAssert(t, "delete", "--spec="+deleteSpecPath)
	} else {
		CliExecAndAssert(t, "delete", tests.RtRepo1+"/test_resources/b/*")
	}
...
func TestArtifactoryDeleteFolderWithWildcard(t *testing.T) {
	...
	CliExecAndAssert(t, "delete", tests.RtRepo1+"/test_resources/*/c")
...
func testArtifactoryDeleteFoldersNoSpec(t *testing.T, contentOnly bool) {
	...
	CliExecAndAssert(t, "delete", deletePath)
...
func TestArtifactoryDeleteExcludeByCli(t *testing.T) {
	...
	// Delete by pattern
	CliExecAndAssert(t, "del", tests.RtRepo1+"/data/", "--exclusions=*/*b1.in;*/*b2.in;*/*b3.in;*/*c1.in")
...
func TestArtifactoryDeleteExclusionsBySpec(t *testing.T) {
	...
	// Delete by pattern
	CliExecAndAssert(t, "del", "--spec="+specFile)
...
func TestArtifactoryDeletePrefixFiles(t *testing.T) {
	...
	// Delete by pattern
	CliExecAndAssert(t, "delete", tests.RtRepo1+"/*")
...
func TestArtifactoryDeleteByProps(t *testing.T) {
	...
	CliExecAndAssert(t, "sp", tests.RtRepo1+"/a/b/", "D=5", "--include-dirs")
	CliExecAndAssert(t, "sp", tests.RtRepo1+"/a/b/c/", "D=2", "--include-dirs")

	//  Set the property D=5 to c1.in, which is a different value then its directory c/
	CliExecAndAssert(t, "sp", tests.RtRepo1+"/a/b/c/c1.in", "D=5")
	...
	CliExecAndAssert(t, "delete", tests.RtRepo1+"/*", "--props=D=5", "--exclude-props=c=3")
	...
	CliExecAndAssert(t, "delete", tests.RtRepo1+"/*", "--props=c=3", "--exclude-props=a=1")
	...
	CliExecAndAssert(t, "delete", tests.RtRepo1+"/*", "--props=a=1", "--exclude-props=b=3;c=3")
...
func TestArtifactoryFolderUploadRecursiveNonFlat(t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", tests.Out+"/(*)", tests.RtRepo1+"/{1}/", "--include-dirs=true", "--recursive=true", "--flat=false")
	err = os.RemoveAll(tests.Out)
	assert.NoError(t, err)
	CliExecAndAssert(t, "download", tests.RtRepo1, tests.Out+"/", "--include-dirs=true", "--recursive=true")
...
func TestArtifactoryFlatFolderUpload(t *testing.T) {
	...
	err := os.MkdirAll(canonicalPath, 0777)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", tests.Out+"/(*)", tests.RtRepo1+"/{1}/", "--include-dirs=true", "--flat=true")
	err = os.RemoveAll(tests.Out)
	assert.NoError(t, err)
	// Non flat download
	CliExecAndAssert(t, "download", tests.RtRepo1, tests.Out+"/", "--include-dirs=true", "--recursive=true")
...
func TestArtifactoryIncludeDirFlatNonEmptyFolderUpload(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", tests.GetTestResourcesPath()+"a/b/*", tests.RtRepo1, "--include-dirs=true", "--flat=true")
	CliExecAndAssert(t, "download", tests.RtRepo1, tests.Out+"/", "--include-dirs=true", "--recursive=true")
...
func TestArtifactoryDownloadNotIncludeDirs(t *testing.T) {
	initArtifactoryTest(t)
	// 'c' folder is defined as bottom chain directory therefore should be uploaded when using flat=true even though 'c' is not empty
	CliExecAndAssert(t, "upload", tests.GetTestResourcesPath()+"*/c", tests.RtRepo1, "--include-dirs=true", "--flat=true")
	CliExecAndAssert(t, "download", tests.RtRepo1, tests.Out+"/", "--recursive=true")
...
func TestArtifactoryDownloadFlatTrue(t *testing.T) {
	...
	// 'c' folder is defined as bottom chain directory therefore should be uploaded when using flat=true even though 'c' is not empty
	CliExecAndAssert(t, "upload", tests.GetTestResourcesPath()+"(a*)/*", tests.RtRepo1+"/{1}/", "--include-dirs=true", "--flat=true")
	// Download without include-dirs
	CliExecAndAssert(t, "download", tests.RtRepo1, tests.Out+"/", "--recursive=true", "--flat=true")
	assert.False(t, fileutils.IsPathExists(tests.Out+"/c", false), "'c' folder shouldn't exist.")

	CliExecAndAssert(t, "download", tests.RtRepo1, tests.Out+"/", "--include-dirs=true", "--recursive=true", "--flat=true")
...
func TestArtifactoryFlatFolderDownload1(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", tests.Out+"/(*)", tests.RtRepo1+"/{1}/", "--include-dirs=true")
	err = os.RemoveAll(tests.Out)
	assert.NoError(t, err)
	// Only the inner folder should be downland e.g 'folder'
	CliExecAndAssert(t, "download", tests.RtRepo1, tests.Out+"/", "--include-dirs=true", "--flat=true")
...
func TestArtifactoryFolderUploadRecursiveUsingSpec(t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "download", "--spec="+specFile)
...
func TestArtifactoryFolderUploadNonRecursive(t *testing.T) {
	...
	err := os.MkdirAll(canonicalPath, 0777)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", tests.Out+"/", tests.RtRepo1, "--recursive=true", "--include-dirs=true", "--flat=true")
	err = os.RemoveAll(tests.Out)
	assert.NoError(t, err)
	CliExecAndAssert(t, "download", tests.RtRepo1, tests.Out+"/", "--include-dirs=true")
...
func TestArtifactoryFolderDownloadNonRecursive(t *testing.T) {
	...
	err := os.MkdirAll(canonicalPath, 0777)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", tests.Out+"/", tests.RtRepo1, "--recursive=true", "--include-dirs=true", "--flat=false")
	err = os.RemoveAll(tests.Out)
	assert.NoError(t, err)
	CliExecAndAssert(t, "download", tests.RtRepo1+"/*", "--recursive=false", "--include-dirs=true")
...
func TestArtifactoryChecksumDownload(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1, "--flat=true")
...
func testChecksumDownload(t *testing.T, outFileName string) {
	CliExecAndAssert(t, "download", tests.RtRepo1+"/a1.in", tests.Out+outFileName)
...
func TestArtifactoryDownloadByPatternAndBuildUsingSpec(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "--spec="+specFileB, "--build-name="+tests.RtBuildName1, "--build-number="+buildNumberB)

	// Publish buildInfo
	CliExecAndAssert(t, "build-publish", tests.RtBuildName1, buildNumberA)
	CliExecAndAssert(t, "build-publish", tests.RtBuildName1, buildNumberB)
...
func prepareDownloadByBuildWithDependenciesTests(t *testing.T) {
	...
	// Add build artifacts.
	CliExecAndAssert(t, "upload", "--spec="+specFileA, "--build-name="+tests.RtBuildName1, "--build-number="+buildNumber)
	CliExecAndAssert(t, "upload", "--spec="+specFileB)
...
func TestArtifactoryDownloadByBuildWithDependencies(t *testing.T) {
	...
	// Download with exclude-artifacts.
	CliExecAndAssert(t, "download", tests.RtRepo1, "out/download/download_build_with_dependencies/", "--build="+tests.RtBuildName1, "--exclude-artifacts=true", "--flat=true")
	// Validate.
	paths, _ := fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	err := tests.ValidateListsIdentical(tests.GetBuildDownloadDoesntExist(), paths)
	assert.NoError(t, err)
	...
	// Download deps-only.
	CliExecAndAssert(t, "download", tests.RtRepo1, "out/download/download_build_only_dependencies/", "--build="+tests.RtBuildName1, "--exclude-artifacts=true", "--include-deps=true", "--flat=true")
	// Validate.
	paths, _ = fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	err = tests.ValidateListsIdentical(tests.GetDownloadByBuildOnlyDeps(), paths)
	assert.NoError(t, err)
	...
	// Download artifacts and deps.
	CliExecAndAssert(t, "download", tests.RtRepo1, "out/download/download_build_with_dependencies/", "--build="+tests.RtBuildName1, "--include-deps=true", "--flat=true")
	// Validate.
	paths, _ = fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out+string(os.PathSeparator)+"download"+string(os.PathSeparator)+"download_build_with_dependencies", false)
	err = tests.ValidateListsIdentical(tests.GetDownloadByBuildIncludeDeps(), paths)
	assert.NoError(t, err)
...
func TestArtifactoryDownloadArtifactDoesntExistInBuild(t *testing.T) {
	...
	assert.NoError(t, err)
	// Upload a file
	CliExecAndAssert(t, "upload", "testdata/a/a1.in", tests.RtRepo1+"/data/a10.in", "--build-name="+tests.RtBuildName1, "--build-number="+buildNumber)
...
func TestArtifactoryDownloadByShaAndBuild(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/a/a1.in", tests.RtRepo1+"/data/a10.in", "--build-name="+tests.RtBuildName2, "--build-number="+buildNumberA)
	CliExecAndAssert(t, "upload", "testdata/a/a1.in", tests.RtRepo1+"/data/a11.in", "--build-name="+tests.RtBuildName1, "--build-number="+buildNumberB)
	CliExecAndAssert(t, "upload", "testdata/a/a1.in", tests.RtRepo1+"/data/a12.in", "--build-name="+tests.RtBuildName1, "--build-number="+buildNumberC)

	// Publish buildInfo
	CliExecAndAssert(t, "build-publish", tests.RtBuildName2, buildNumberA)
	CliExecAndAssert(t, "build-publish", tests.RtBuildName1, buildNumberB)
	CliExecAndAssert(t, "build-publish", tests.RtBuildName1, buildNumberC)
...
func TestArtifactoryDownloadByShaAndBuildName(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/a/a1.in", tests.RtRepo1+"/data/a10.in", "--build-name="+tests.RtBuildName2, "--build-number="+buildNumberA)
	CliExecAndAssert(t, "upload", "testdata/a/a1.in", tests.RtRepo1+"/data/a11.in", "--build-name="+tests.RtBuildName2, "--build-number="+buildNumberB)
	CliExecAndAssert(t, "upload", "testdata/a/a1.in", tests.RtRepo1+"/data/a12.in", "--build-name="+tests.RtBuildName1, "--build-number="+buildNumberC)

	// Publish buildInfo
	CliExecAndAssert(t, "build-publish", tests.RtBuildName1, buildNumberC)
	CliExecAndAssert(t, "build-publish", tests.RtBuildName2, buildNumberA)
	CliExecAndAssert(t, "build-publish", tests.RtBuildName2, buildNumberB)
...
func TestArtifactoryDownloadByBuildUsingSimpleDownload(t *testing.T) {
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/data/a1.in", filepath.Join(tests.Out, "download", "simple_by_build")+fileutils.GetFileSeparator(), "--build="+tests.RtBuildName1)
	CliExecAndAssert(t, "download", tests.RtRepo1+"/data/b1.in", filepath.Join(tests.Out, "download", "simple_by_build")+fileutils.GetFileSeparator(), "--build="+tests.RtBuildName1)
...
func TestArtifactoryDownloadByBuildUsingSimpleDownloadWithProject(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "--spec="+specFileB, "--build-name="+tests.RtBuildName1, "--build-number="+buildNumberA, "--project="+projectKey)

	// Publish buildInfo with project flag
	CliExecAndAssert(t, "build-publish", tests.RtBuildName1, buildNumberA, "--project="+projectKey)

	// Download by project, b1 should be downloaded
	CliExecAndAssert(t, "download", tests.RtRepo1+"/data/b1.in", filepath.Join(tests.Out, "download", "simple_by_build")+fileutils.GetFileSeparator(),
...
func TestArtifactoryDownloadByBuildNoPatternUsingSimpleDownload(t *testing.T) {
	...
	// Download by build number, a1 should not be downloaded, b1 should
	CliExecAndAssert(t, "download", "*", filepath.Join(tests.Out, "download", "simple_by_build")+fileutils.GetFileSeparator(), "--build="+tests.RtBuildName1+"/"+buildNumberA)
...
func TestArtifactoryDownloadExcludeByCli(t *testing.T) {
	...
	specFileB, err := tests.CreateSpec(tests.SplitUploadSpecB)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+specFileA, "--recursive=true")
	CliExecAndAssert(t, "upload", "--spec="+specFileB, "--recursive=true")

	// Download by pattern
	CliExecAndAssert(t, "download", tests.RtRepo1, "out/download/aql_by_artifacts/", "--exclusions=*/*/a1.in;*/*a2.*;*/data/c2.in")
...
func TestArtifactoryDownloadExclusionsBySpec(t *testing.T) {
	...
	specFileB, err := tests.CreateSpec(tests.SplitUploadSpecB)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+specFileA, "--recursive=true", "--flat=false")
	CliExecAndAssert(t, "upload", "--spec="+specFileB, "--recursive=true", "--flat=false")
...
func TestArtifactoryDownloadExclusionsBySpecOverride(t *testing.T) {
	...
	specFile, err := tests.CreateSpec(tests.DownloadSpecExclusions)
	assert.NoError(t, err)
	CliExecAndAssert(t, "download", "--spec="+specFile, "--exclusions=*a1.in;*a2.in;*c2.in")
...
func TestArtifactoryLimitWithSymlink(t *testing.T) {
	...
	err := os.Symlink(localFile, link)
	assert.NoError(t, err)
	CliExecAndAssert(t, "u", link, tests.RtRepo1, "--symlinks=true", "--flat=true")
	err = os.Remove(link)
	assert.NoError(t, err)
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/link", tests.GetTestResourcesPath()+"a/", "--validate-symlinks=true", "--limit=1")
...
func TestArtifactorySortWithSymlink(t *testing.T) {
	...
	err = os.Remove(link)
	assert.NoError(t, err)
	CliExecAndAssert(t, "dl", tests.RtRepo1+"/link", tests.GetTestResourcesPath()+"a/", "--validate-symlinks=true", "--sort-by=created")
	validateSortLimitWithSymLink(link, localFile, t)
	assert.NoError(t, os.Remove(link))
	cleanArtifactoryTest()
}
...
func TestArtifactoryDownloadByShaAndBuildNameWithSort(t *testing.T) {
	...
	// Download by build number
	CliExecAndAssert(t, "download", "--sort-by=created", "--spec="+specFile)
...
func TestArtifactoryCopyByBuildUsingSpec(t *testing.T) {
	...
	CliExecAndAssert(t, "copy", "--spec="+specFile)
...
func TestArtifactorySortAndLimit(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/a/(*)", tests.RtRepo1+"/data/{1}")

	// Download 1 sorted by name asc
	CliExecAndAssert(t, "download", tests.RtRepo1+"/data/", "out/download/sort_limit/", "--sort-by=name", "--limit=1")

	// Download 3 sorted by depth desc
	CliExecAndAssert(t, "download", tests.RtRepo1+"/data/", "out/download/sort_limit/", "--sort-by=depth", "--limit=3", "--sort-order=desc")
...
func TestArtifactorySortByCreated(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", "testdata/created/or", tests.RtRepo1, `--target-props=k1=v1`, "--flat=true")
	CliExecAndAssert(t, "upload", "testdata/created/o", tests.RtRepo1, "--flat=true")
	CliExecAndAssert(t, "upload", "testdata/created/org", tests.RtRepo1, "--flat=true")
...
func TestArtifactoryOffset(t *testing.T) {
	...
	// Upload all testdata/a/ files
	CliExecAndAssert(t, "upload", "testdata/a/*", path.Join(tests.RtRepo1, "offset_test")+"/", "--flat=true")
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/offset_test/", tests.Out+"/", "--flat=true", "--sort-by=name", "--limit=1", "--offset=0")
	paths, _ := fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	...
	CliExecAndAssert(t, "download", tests.RtRepo1+"/offset_test/", tests.Out+"/", "--flat=true", "--sort-by=name", "--limit=1", "--offset=1")
	paths, _ = fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
	tests.VerifyExistLocally([]string{filepath.Join(tests.Out, "a2.in")}, paths, t)

	// Download the third file, expecting to download a3.in
	CliExecAndAssert(t, "download", tests.RtRepo1+"/offset_test/", tests.Out+"/", "--flat=true", "--sort-by=name", "--limit=1", "--offset=2")
	paths, _ = fileutils.ListFilesRecursiveWalkIntoDirSymlink(tests.Out, false)
...
func TestArtifactoryCopyByBuildOverridingByInlineFlag(t *testing.T) {
	...
	specFileB, err := tests.CreateSpec(tests.SplitUploadSpecB)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+specFileB, "--build-name="+tests.RtBuildName1, "--build-number="+buildNumberA)
	CliExecAndAssert(t, "upload", "--spec="+specFileA, "--build-name="+tests.RtBuildName1, "--build-number="+buildNumberB)
	...
	CliExecAndAssert(t, "copy", "--build="+tests.RtBuildName1, "--spec="+specFile)
...
func TestArtifactoryMoveByBuildUsingFlags(t *testing.T) {
	...
	CliExecAndAssert(t, "move", "--build="+tests.RtBuildName1+"/11", "--spec="+specFile)
...
func TestArtifactoryMoveExcludeByCli(t *testing.T) {
	...
	// Move by pattern
	CliExecAndAssert(t, "move", tests.RtRepo1+"/data/", tests.RtRepo2+"/", "--exclusions=*/*b*;*/*c*")
...
func TestArtifactoryMoveExclusionsBySpec(t *testing.T) {
	...
	// Move by spec
	CliExecAndAssert(t, "move", "--spec="+specFile)
...
func TestArtifactoryDeleteByLatestBuild(t *testing.T) {
	...
	CliExecAndAssert(t, "delete", "--build="+tests.RtBuildName1+"/LATEST", "--spec="+specFile)
...
func TestGitLfsCleanup(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtLfsRepo+"/objects/4b/f4/{2}{1}", "--flat=true")
	CliExecAndAssert(t, "upload", filePath, tests.RtLfsRepo+"/objects/4b/f4/", "--flat=true")
	refs := filepath.Join("refs", "heads", "*")
	dotGitPath := getCliDotGitPath(t)
	CliExecAndAssert(t, "glc", dotGitPath, "--repo="+tests.RtLfsRepo, "--refs=HEAD,"+refs)
...
func TestArtifactoryBuildDiscard(t *testing.T) {
	...
	// Upload files with buildName and buildNumber
	for i := 1; i <= 5; i++ {
		CliExecAndAssert(t, "upload", "testdata/a/a1.in", tests.RtRepo1+"/data/", "--build-name="+tests.RtBuildName1, "--build-number="+strconv.Itoa(i))
		CliExecAndAssert(t, "build-publish", tests.RtBuildName1, strconv.Itoa(i))
	}

	// Test discard by max-builds
	CliExecAndAssert(t, "build-discard", tests.RtBuildName1, "--max-builds=3")
	...
	CliExecAndAssert(t, "build-discard", tests.RtBuildName1, "--max-days=-1", "--exclude-builds=3,5")
	jsonResponse = getAllBuildsByBuildName(client, tests.RtBuildName1, t, http.StatusOK)
	assert.Len(t, jsonResponse.Builds, 2, "Incorrect operation of build-discard with exclusion.")

	// Test discard by max-days
	CliExecAndAssert(t, "build-discard", tests.RtBuildName1, "--max-days=-1")
...
func TestArtifactoryWinBackwardsCompatibility(t *testing.T) {
	...
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+uploadSpecFile)
	CliExecAndAssert(t, "upload", "testdata\\\\a\\\\b\\\\*", tests.RtRepo1+"/compatibility_arguments/", "--exclusions=*b2.in;*c*")

	downloadSpecFile, err := tests.CreateSpec(tests.WinSimpleDownloadSpec)
	assert.NoError(t, err)
	CliExecAndAssert(t, "download", "--spec="+downloadSpecFile)
	CliExecAndAssert(t, "download", tests.RtRepo1+"/*arguments*", "out\\\\win\\\\", "--flat=true")
...
func TestArtifactorySearchIncludeDir(t *testing.T) {
	...
	specFileA, err := tests.CreateSpec(tests.SplitUploadSpecA)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+specFileA, "--recursive", "--flat=false")
...
func TestArtifactorySearchProps(t *testing.T) {
	...
	specFile, err := tests.CreateSpec(tests.UploadWithPropsSpec)
	assert.NoError(t, err)
	CliExecAndAssert(t, "upload", "--spec="+specFile, "--recursive")
...
func TestArtifactoryDeleteExcludeProps(t *testing.T) {
	...
	CliExecAndAssert(t, "delete", tests.RtRepo1+"/*", "--exclude-props=c=1")
...
func preUploadBasicTestResources(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", uploadPath, targetPath,
...
func testCopyMoveNoSpec(command string, beforeCommandExpected, afterCommandExpected []string, t *testing.T) {
	...
	CliExecAndAssert(t, command, tests.RtRepo1+"/data/*a*", tests.RtRepo2+"/", "--dry-run")
	...
	// Run command
	CliExecAndAssert(t, command, tests.RtRepo1+"/data/*a*", tests.RtRepo2+"/")
...
func TestArtifactoryUploadInflatedPath(t *testing.T) {
	initArtifactoryTest(t)
	CliExecAndAssert(t, "upload", "testdata/a/../a/a1.*", tests.RtRepo1, "--flat=true")
	CliExecAndAssert(t, "upload", "testdata/./a/a1.*", tests.RtRepo1, "--flat=true")
	searchFilePath, err := tests.CreateSpec(tests.SearchRepo1ByInSuffix)
	assert.NoError(t, err)
	verifyExistInArtifactory(tests.GetSimpleUploadSpecialCharNoRegexExpectedRepo1(), searchFilePath, t)

	CliExecAndAssert(t, "upload", "testdata/./a/../a/././././a2.*", tests.RtRepo1, "--flat=true")
	searchFilePath, err = tests.CreateSpec(tests.SearchRepo1ByInSuffix)
	assert.NoError(t, err)
	verifyExistInArtifactory(tests.GetSimpleUploadSpecialCharNoRegexExpected2filesRepo1(), searchFilePath, t)
	if coreutils.IsWindows() {
		CliExecAndAssert(t, "upload", `testdata\\a\\..\\a\\a1.*`, tests.RtRepo2, "--flat=true")
		CliExecAndAssert(t, "upload", `testdata\\.\\\a\a1.*`, tests.RtRepo2, "--flat=true")
		...
		CliExecAndAssert(t, "upload", `testdata\\.\\a\\..\\a\\.\\.\\.\\.\\a2.*`, tests.RtRepo2, "--flat=true")
		searchFilePath, err = tests.CreateSpec(tests.SearchRepo2)
		assert.NoError(t, err)
...
func TestVcsProps(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filepath.Join(testDir, "*"), tests.RtRepo1, "--flat=false", "--build-name="+tests.RtBuildName1, "--build-number=2020")
	resultItems := searchItemsInArtifactory(t, tests.SearchRepo1ByInSuffix)
	assert.NotZero(t, len(resultItems), "No artifacts were found.")
	for _, item := range resultItems {
		properties := item.Properties
...
func TestArtifactoryReplicationCreate(t *testing.T) {
	...
	CliExecAndAssert(t, "rplc", specFile)
	...
	// Delete replication
	CliExecAndAssert(t, "rpldel", tests.RtRepo1)
...
func TestAccessTokenCreate(t *testing.T) {
	...
	CliExecAndAssert(t, "atc", *tests.JfrogUser)
...
func uploadUsingAntAndRegexpTogether(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1, "--regexp", "--ant", "--flat=true")
}

func simpleUploadAntIsTrueRegexpIsFalse(t *testing.T) {
	filePath := getAntPatternFilePath()
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1, "--ant", "--regexp=false", "--flat=true")
	searchFilePath, err := tests.CreateSpec(tests.SearchRepo1ByInSuffix)
	assert.NoError(t, err)
...
func uploadUsingAntAIncludeDirsAndFlat(t *testing.T) {
	...
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1, "--ant", "--include-dirs=true", "--flat=true")
	CliExecAndAssert(t, "upload", filePath, tests.RtRepo1, "--ant", "--include-dirs=true", "--flat=false")
...
func TestPermissionTargets(t *testing.T) {
	...
	CliExecAndAssert(t, "ptc", templatePath, createPermissionTargetsTemplateVars(tests.RtRepo1))
	assertPermissionTarget(t, servicesManager, tests.RtRepo1)

	// Update permission target to ANY repo.
	any := "ANY"
	CliExecAndAssert(t, "ptu", templatePath, createPermissionTargetsTemplateVars(any))
	assertPermissionTarget(t, servicesManager, any)

	// Delete permission target.
	CliExecAndAssert(t, "ptdel", tests.RtPermissionTargetName)
