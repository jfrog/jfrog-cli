...
func TestConfigEncryptionMissingKey(t *testing.T) {
	...
	assert.NoError(t, artifactoryCli.WithoutCredentials().Exec("ping", "--server-id=encryption-server-id"))
...
// TestBasicAuthRefreshTokenAndPing verifies the username/password (basic auth) flow end to end:
// configuring a server with user + password arms deferred refreshable-token creation, and the first
// command (ping) authenticates successfully and triggers creation of the refreshable Artifactory token.
func TestBasicAuthRefreshTokenAndPing(t *testing.T) {
	initArtifactoryTest(t, "")

	// This flow is only relevant for username/password authentication.
	if *tests.JfrogAccessToken != "" {
		t.Skip("Test only with username and password, skipping...")
	}

	// Configure a server with username + password. No token is created at this point
	_, err := createServerConfigAndReturnPassphrase(t)
	defer deleteServerConfig(t)
	assert.NoError(t, err)

	// Right after 'config add' there is no refreshable token yet, only the armed refresh interval.
	preDetails, err := config.GetSpecificConfig(tests.ServerId, false, false)
	assert.NoError(t, err)
	assert.Empty(t, preDetails.AccessToken, "no access token should exist before the first command")
	assert.Empty(t, preDetails.ArtifactoryRefreshToken, "no refresh token should exist before the first command")
	assert.Equal(t, coreutils.TokenRefreshDefaultInterval, preDetails.ArtifactoryTokenRefreshInterval,
		"the refresh interval should be armed so the token is created on the first command")

	// Run ping using only the stored server-id (no inline credentials), so the CLI authenticates with
	// the configured username/password and creates the refreshable token as part of the command.
	err = coretests.NewJfrogCli(execMain, "jfrog rt", "--server-id="+tests.ServerId).Exec("ping")
	assert.NoError(t, err, "ping should succeed using the configured username/password")

	// Assert the refreshable Artifactory token was generated by the basic-auth flow.
	curAccessToken, curRefreshToken, curArtifactoryRefreshToken, err := getTokensFromConfig(t)
	assert.NoError(t, err)
	assert.NotEmpty(t, curAccessToken, "an access token should be created after the first command")
	assert.NotEmpty(t, curArtifactoryRefreshToken, "an Artifactory refresh token should be created after the first command")
	assert.Empty(t, curRefreshToken, "the access refresh token must remain empty for the Artifactory-API flow")
