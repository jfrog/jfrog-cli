...
func GetCommands() []cli.Command {
	return []cli.Command{
		...
		{
			Name:      "go-recursive-tidy",
			Flags:     getGoRecursiveTidyFlags(),
			Aliases:   []string{"grt"},
			Usage:     gocommand.Description,
			HelpName:  common.CreateUsage("rt grt", gocommand.Description, gocommand.Usage),
			UsageText: gocommand.Arguments,
			ArgsUsage: common.CreateEnvVars(),
			Action: func(c *cli.Context) {
				goRecursiveTidyCmd(c)
			},
		},
		{
			Name:      "ping",
...
func getGoFlags() []cli.Flag {
	...
	flags = append(flags, getRecursiveTidyOverwriteFlag())
...
func getGoRecursiveTidyFlags() []cli.Flag {
	flags := append(getBaseFlags(), getRecursiveTidyOverwriteFlag())
	return append(flags, getServerIdFlag())
}

func getRecursiveTidyOverwriteFlag() cli.Flag {
	return cli.BoolFlag{
...
func goRecursiveTidyCmd(c *cli.Context) {
	targetRepo := c.Args().Get(0)
	if targetRepo == "" {
		cliutils.PrintHelpAndExitWithError("Missing target repo.", c)
	}
	details := createArtifactoryDetailsByFlags(c, true)
	err := gocmd.SetGoProxyEnvVar(details.GetUrl(), details.GetUser(), details.GetPassword(), targetRepo)
	if err != nil {
		cliutils.ExitOnErr(err)
	}
	...
	goModEditMessage := "// Generated by GoCenter"
	if !exists {
		emptyFile, err := os.Create("go.mod")
		if err != nil {
			cliutils.ExitOnErr(err)
		}
		emptyFile.Close()
		err = gocmd.RunGoModTidy(goModEditMessage, true)
		if err != nil {
			cliutils.ExitOnErr(err)
		}
	}
	...
	tidyEnum := gocmd.TidyEnum{}
	if c.Bool("recursive-tidy-overwrite") {
		tidyEnum.SetRecursiveTidyOverwrite()
	} else {
		tidyEnum.SetRecursiveTidy()
	}
	err = project.DownloadFromVcsAndPublish(targetRepo, "", goModEditMessage, tidyEnum, serviceManager)
	if err != nil {
		cliutils.ExitOnErr(err)
	}
}
