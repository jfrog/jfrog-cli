...
func TestPoetryInstallGroupFilters(t *testing.T) {
	initPoetryTest(t)

	// FlexPack routing must be deterministic regardless of ambient env.
	restoreEnv := clientTestUtils.SetEnvWithCallbackAndAssert(t, "JFROG_RUN_NATIVE", "true")
	defer restoreEnv()

	// FlexPack shells out to `jf config show` (needs a `jf` on PATH) and to
	// `poetry` (needs auth against the Artifactory PyPI virtual repo).
	restorePath := buildJfBinaryAndAddToPath(t)
	defer restorePath()
	restoreAuth := setPoetryHTTPBasicAuth(t)
	defer restoreAuth()
	...
	cases := []struct {
		name         string
		installArgs  []string
		wantDeps     []string // package names that MUST be present in build-info
		unwantedDeps []string // package names that MUST be absent from build-info
	}{
		{
			name:         "only-main-excludes-dev",
			installArgs:  []string{"--only", "main"},
			wantDeps:     []string{"requests"},
			unwantedDeps: []string{"pytest", "black"},
		},
		{
			name:         "without-dev-excludes-dev",
			installArgs:  []string{"--without", "dev"},
			wantDeps:     []string{"requests"},
			unwantedDeps: []string{"pytest", "black"},
		},
		{
			name:         "with-dev-includes-dev",
			installArgs:  []string{"--with", "dev"},
			wantDeps:     []string{"requests", "pytest", "black"},
			unwantedDeps: nil,
		},
	}
	...
	for i, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			projectPath := createPoetryProject(t, "group-filters-"+tc.name, "poetryproject")
			...
			// Isolate the Poetry cache per case to avoid cross-test contamination.
			tmpDir, createTempDirCallback := coretests.CreateTempDirWithCallbackAndAssert(t)
			defer createTempDirCallback()
			unsetCache := clientTestUtils.SetEnvWithCallbackAndAssert(t, "POETRY_CACHE_DIR", filepath.Join(tmpDir, "cache"))
			defer unsetCache()

			args := append([]string{"poetry", "install"}, tc.installArgs...)
			args = append(args, "--build-name="+tests.PoetryBuildName, "--build-number="+buildNumber)

			jfrogCli := coretests.NewJfrogCli(execMain, "jfrog", "")
			require.NoError(t, jfrogCli.Exec(args...))

			// Publish and fetch the build info.
			require.NoError(t, artifactoryCli.Exec("bp", tests.PoetryBuildName, buildNumber))
			...
			// Collect the set of dependency package names (Id is "name:version").
			...
			for _, dep := range publishedBuildInfo.BuildInfo.Modules[0].Dependencies {
				...
				depNames[strings.ToLower(name)] = true
			}

			for _, want := range tc.wantDeps {
				assert.Truef(t, depNames[want], "expected dependency %q in build-info for `poetry install %v`, got %v",
					want, tc.installArgs, depNames)
			}
			for _, unwanted := range tc.unwantedDeps {
				assert.Falsef(t, depNames[unwanted], "dependency %q must be EXCLUDED from build-info for `poetry install %v`, got %v",
					unwanted, tc.installArgs, depNames)
			}
