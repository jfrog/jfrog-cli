...
var RepoDefaultName = map[techindicators.Technology]map[string]string{
	techindicators.Maven: {
		Local:   MavenLocalDefaultName,
		Remote:  MavenRemoteDefaultName,
		Virtual: MavenVirtualDefaultName,
	},
	techindicators.Gradle: {
...
var RepoRemoteDefaultUrl = map[techindicators.Technology]string{
	techindicators.Maven:  MavenRemoteDefaultUrl,
	techindicators.Gradle: GradleRemoteDefaultUrl,
	techindicators.Npm:    NpmRemoteDefaultUrl,
}

var buildCmdByTech = map[techindicators.Technology]string{
	techindicators.Maven:  mavenDefaultBuildCmd,
	techindicators.Gradle: gradleDefaultBuildCmd,
	techindicators.Npm:    npmDefaultBuildCmd,
}
...
func CreateLocalRepo(serviceDetails *utilsconfig.ServerDetails, technologyType techindicators.Technology, repoName string) error {
	servicesManager, err := artUtils.CreateServiceManager(serviceDetails, -1, false)
	if err != nil {
		return err
	}
...
func CreateRemoteRepo(serviceDetails *utilsconfig.ServerDetails, technologyType techindicators.Technology, repoName, remoteUrl string) error {
	servicesManager, err := artUtils.CreateServiceManager(serviceDetails, -1, false)
	if err != nil {
		return err
	}
...
func CreateVirtualRepo(serviceDetails *utilsconfig.ServerDetails, technologyType techindicators.Technology, repoName string, repositories ...string) error {
	servicesManager, err := artUtils.CreateServiceManager(serviceDetails, -1, false)
	if err != nil {
		return err
	}
