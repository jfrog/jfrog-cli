...
// into the agent's global harness directory.
// Covers scenario #26.
func TestAgentPluginsInstallGlobal(t *testing.T) {
	...
	// Install globally via --path to a controlled temp directory (avoids touching
	// the real home directory on the CI runner).
	...
	assert.DirExists(t, filepath.Join(globalBase, slug),
		"globally installed plugin should be at <base>/<slug>")
}
...
// TestAgentPluginsChecksumRoundTrip publishes a plugin, installs it, and
// verifies the local SHA256 of the installed zip matches what Artifactory stored.
// Covers scenario #70.
func TestAgentPluginsChecksumRoundTrip(t *testing.T) {
	initAgentPluginsTest(t)
	defer cleanAgentPluginsTest()
...
func TestAgentPluginsNoProxy(t *testing.T) {
	...
	restoreNoProxy := clientTestUtils.SetEnvWithCallbackAndAssert(t, "NO_PROXY", "*")
	defer restoreNoProxy()
...
func TestAgentPluginsListCheckUpdates(t *testing.T) {
	...
	// --check-updates requires --harness, not --path; use the published repo for comparison.
	assert.NoError(t, runAgentPluginsCmd(t,
		"list",
		"--repo="+tests.AgentPluginsLocalRepo,
		"--check-updates",
	), "list --check-updates --repo should run without error")
}
...
func TestAgentPluginsListFlags(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			expectError: true,
			description: "--sort-order with invalid value must error",
		},
		{
			name:        "check-updates-without-harness",
			args:        []string{"list", "--repo=" + tests.AgentPluginsLocalRepo, "--check-updates"},
			expectError: false,
			description: "--check-updates with --repo should succeed (compares remote versions)",
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
